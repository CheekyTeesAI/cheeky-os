...
function getNextBestCashActions(input) {
  ...
  const pendingFollowup =
    Number(queue.pendingFollowup) || Number(queue.pending_followup) || 0;
  ...
  if (approved > 0 && !autoSend) add("Send approved outreach now");
  if (hotLeads > 0 && pending > 0) add("Review and approve hot leads first");
  if (failed > 0) add("Retry failed outreach");
  if (pendingFollowup > 0) add("Review follow-up queue");
  if (processed === 0) add("Refresh or reconnect lead source");
  if (sent > 0) add("Check inbox for replies and convert interested leads");
  if (!autoSend && dryRun) {
