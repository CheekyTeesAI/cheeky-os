import type { ActiveLead } from "../production/getActiveLeads";
import type { Job } from "../production/getActiveOrders";
import type { ActiveTask } from "../production/getActiveTasks";
...
function leadStale(lead: ActiveLead): boolean {
  if (!lead.lastActivityDate) return true;
  const t = new Date(lead.lastActivityDate).getTime();
  if (!Number.isFinite(t)) return true;
  return Date.now() - t > 48 * 60 * 60 * 1000;
}

function orderAction(scored: ScoredOrder): DecisionAction {
