...
import type { FollowUpCandidate, FollowUpCandidateType } from "./followUpScoring";

export type OutreachDraft = {
  followUpType: FollowUpCandidateType;
...
function orderRef(c: FollowUpCandidate): string {
 ...
 */
export function buildOutreachDraft(c: FollowUpCandidate): OutreachDraft {
  const greet = firstName(c.customerName);
  const ref =
    c.sourceType === "ORDER" ? orderRef(c) : "your Cheeky Tees account";
...
Cheeky Tees`;
  ...
  const reviewRequired =
    c.reviewRequired || c.type === "MANUAL_REVIEW";
...
export function buildOutreachDraftsForCandidates(
