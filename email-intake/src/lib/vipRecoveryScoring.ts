...
export type VipRecoveryOrderFact = {
  id: string;
  createdAt: Date;
  updatedAt: Date;
  amountPaid: number;
...
export type VipRecoveryCustomerRollup = {
  customerId: string;
  customerName: string;
  customerEmail: string | null;
  customerPhone: string | null;
  orders: VipRecoveryOrderFact[];
};

export type VipRecoveryCandidate = {
  customerId: string;
  customerName: string;
  customerEmail: string | null;
  customerPhone: string | null;
  ...
  estimatedHistoricalSpend: number;
  largestKnownOrderValue: number;
  repeatCustomer: boolean;
  avgHistoricalOrderGapDays: number | null;
  /** Current dormancy vs typical gap (null if not computable) */
  dormancyVsTypicalRatio: number | null;
...
export const VIP_MIN_DORMANCY_DAYS = 45;

/** Below this dormancy, customer is out of scope for this report (too hot). */
export const VIP_TOO_RECENT_DAYS = 30;

function orderValue(o: VipRecoveryOrderFact): number {
...
function paid(o: VipRecoveryOrderFact): number {
...
function hasMaterialOpenBalance(o: VipRecoveryOrderFact): boolean {
...
function hasBlockedOrder(orders: VipRecoveryOrderFact[]): boolean {
  return orders.some(
    (o) =>
...
function computeInterOrderStats(orders: VipRecoveryOrderFact[]): {
  last: VipRecoveryOrderFact;
  first: VipRecoveryOrderFact;
 ...
 */
export function scoreVipRecoveryCustomer(
  rollup: VipRecoveryCustomerRollup
): VipRecoveryCandidate {
  const orders = rollup.orders.filter((o) => o && o.id);
  const factors: string[] = [];

  const weakEmail =
    !rollup.customerEmail || !String(rollup.customerEmail).includes("@");
  ...
  for (const o of orders) {
    ...
    if (ov > largest) largest = ov;
  }

  const repeat = orders.length >= 2;
  let dormancyRatio: number | null = null;
  ...
  if (daysSinceLast < VIP_TOO_RECENT_DAYS) {
    return buildExcluded(
      ...
      dormancyRatio,
      "too_recent",
      `Last order activity ~${Math.floor(daysSinceLast)}d ago — too recent for VIP recovery list.`,
      factors
    );
  }
  ...
  if (lifetimePaid < EPS) {
    return buildExcluded(
      ...
      dormancyRatio,
      "no_paid_history",
      "No recorded payments on file — not a whale recovery candidate.",
      factors
    );
  }

  if (daysSinceLast < VIP_MIN_DORMANCY_DAYS) {
    return buildExcluded(
      rollup,
      daysSinceLast,
      lifetimePaid,
      ...
      dormancyRatio,
      "not_dormant_enough",
      `Only ~${Math.floor(daysSinceLast)}d since last activity; VIP track starts after ~${VIP_MIN_DORMANCY_DAYS}d quiet.`,
      factors
    );
  }
  ...
  if (delinquent) {
    return buildExcluded(
      ...
      dormancyRatio,
      "open_balance",
      "Open balance / collection-sensitive state — reconcile money path before VIP-style outreach.",
      factors
    );
  }
  ...
  if (blocked) {
    return buildExcluded(
      ...
      dormancyRatio,
      "blocked_order",
      "BLOCKED / blockedReason on an order — clear ops blockers first.",
      factors
    );
  }
  ...
  const dormPts = Math.min(16, Math.max(0, daysSinceLast - VIP_MIN_DORMANCY_DAYS) / 6);
  score += dormPts;
  factors.push(
    `dormancy ~${Math.floor(daysSinceLast)}d since last touch (+${dormPts.toFixed(1)} /16)`
  );
  ...
  if (dormancyRatio != null && dormancyRatio >= 2.2) {
    ...
    score += gapPts;
    factors.push(
      `quiet vs typical pattern ×${dormancyRatio.toFixed(2)} (+${gapPts.toFixed(1)} /8)`
    );
  }

  score = Math.max(0, Math.round(score * 10) / 10);

  let tier: VipRecoveryTier;
  if (weakEmail) {
  ...
  return {
    ...
    avgHistoricalOrderGapDays:
      avgGapDays != null ? Math.round(avgGapDays * 10) / 10 : null,
    dormancyVsTypicalRatio:
      dormancyRatio != null ? Math.round(dormancyRatio * 100) / 100 : null,
...
function buildExcluded(
  ...
  return {
    ...
    customerPhone: rollup.customerPhone,
    daysSinceLastOrder:
      Number.isFinite(daysSinceLast) ? Math.round(daysSinceLast * 10) / 10 : null,
...
function suggestedForTier(
  tier: VipRecoveryTier,
  weakEmail: boolean
): string {
  if (weakEmail) return "manual review";
  switch (tier) {
    case "TIER_1_WHALE":
      return "owner call";
...
export function rankVipRecoveryCandidates(
...
export function groupVipRecoveryByTier(
  candidates: VipRecoveryCandidate[]
): Record<VipRecoveryTier, VipRecoveryCandidate[]> {
  const acc = {
    TIER_1_WHALE: [] as VipRecoveryCandidate[],
    TIER_2_HIGH_VALUE: [] as VipRecoveryCandidate[],
    TIER_3_WORTH_REVIEW: [] as VipRecoveryCandidate[],
    REVIEW_REQUIRED: [] as VipRecoveryCandidate[],
    EXCLUDE: [] as VipRecoveryCandidate[],
  };
