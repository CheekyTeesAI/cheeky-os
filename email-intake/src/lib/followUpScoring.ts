...
export type FollowUpCandidateType =
  | "STALE_ESTIMATE"
  | "UNPAID_INVOICE"
  | "CUSTOMER_REACTIVATION"
  | "MANUAL_REVIEW";

export type FollowUpPriorityBand = "HIGH" | "MEDIUM" | "REVIEW_REQUIRED";

export type FollowUpCandidate = {
  type: FollowUpCandidateType;
 ...
 */
export function inferFollowUpTypeFromOrder(input: {
  status: string;
  amountPaid: number;
  totalAmount: number;
  ...
  if (
    ["QUOTE_READY", "APPROVED", "INVOICE_DRAFTED"].includes(st) &&
    total > EPS &&
    paid + EPS < total &&
    input.ageDays >= 2
  ) {
...
function bandFromScore(score: number): FollowUpPriorityBand {
 ...
 */
export function scoreFollowUpCandidate(c: FollowUpCandidate): ScoreBreakdown {
...
export function rankFollowUpCandidates(
...
export function groupByPriorityBand(
  ranked: FollowUpCandidate[]
): {
  HIGH: FollowUpCandidate[];
  MEDIUM: FollowUpCandidate[];
  REVIEW_REQUIRED: FollowUpCandidate[];
} {
  const out = {
    HIGH: [] as FollowUpCandidate[],
    MEDIUM: [] as FollowUpCandidate[],
    REVIEW_REQUIRED: [] as FollowUpCandidate[],
  };
...
export function groupByFollowUpType(
  candidates: FollowUpCandidate[]
): Record<FollowUpCandidateType, FollowUpCandidate[]> {
  const keys: FollowUpCandidateType[] = [
    "STALE_ESTIMATE",
    "UNPAID_INVOICE",
    "CUSTOMER_REACTIVATION",
    "MANUAL_REVIEW",
  ];
  const acc = {} as Record<FollowUpCandidateType, FollowUpCandidate[]>;
