...
import { inferFollowUpTypeFromOrder } from "./followUpScoring";
import type { FollowUpCandidateType } from "./followUpScoring";
...
export type ReorderLikelihood = "LOW" | "MEDIUM" | "HIGH" | "UNKNOWN";

export type QuoteAccelerationCandidate = {
...
export type AccelerationOrderInput = {
  orderId: string;
...
export type CustomerHistoryHint = {
  historicalOrderCount: number;
  estimatedHistoricalSpend: number;
  daysSinceLastOrder: number | null;
...
function mapFollowUpToAcceleration(t: FollowUpCandidateType): AccelerationType | null {
  switch (t) {
    case "STALE_ESTIMATE":
...
function orderValue(o: AccelerationOrderInput): number {
...
function outstanding(o: AccelerationOrderInput): number {
 ...
 */
export function scoreOrderAcceleration(
  o: AccelerationOrderInput,
  history: CustomerHistoryHint
): QuoteAccelerationCandidate | null {
  const st = String(o.status || "").toUpperCase();
  if (st === "CANCELLED") return null;
  if (st === "COMPLETED") return null;
  if (st === "PAID_IN_FULL" && outstanding(o) <= EPS) return null;

  const ageDays =
    (Date.now() - o.updatedAt.getTime()) / 86400000;

  const lane = inferFollowUpTypeFromOrder({
    status: o.status,
  ...
  let score = 0;
  let reorderLikelihood: ReorderLikelihood = "UNKNOWN";
...
export type EasyReorderRollupInput = {
  customerId: string;
  customerName: string;
  customerEmail: string | null;
  customerPhone: string | null;
  ...
  avgOrderGapDays: number | null;
  /** True if customer still has an unpaid quote-lane order — focus quote first. */
  hasOpenQuoteLaneOrder: boolean;
};

/**
 * Repeat customers in a reorder sweet-spot window; skipped if a quote is already open.
 */
export function scoreEasyReorderAcceleration(
  r: EasyReorderRollupInput
): QuoteAccelerationCandidate | null {
  if (r.orderCount < 2) return null;
  if (r.lifetimePaid < 120) return null;
  if (r.hasOpenQuoteLaneOrder) return null;

  const d = r.daysSinceLastOrder;
  if (d < 32 || d > 200) return null;
  ...
  const weakEmail =
    !r.customerEmail || !String(r.customerEmail).includes("@");
  let reorderLikelihood: ReorderLikelihood =
    r.orderCount >= 4 ? "HIGH" : r.orderCount >= 3 ? "MEDIUM" : "LOW";
  if (weakEmail) {
...
export function rankAccelerationCandidates(
  candidates: QuoteAccelerationCandidate[]
): QuoteAccelerationCandidate[] {
...
const TYPE_ORDER: AccelerationType[] = [
  "STALE_QUOTE_REFRESH",
  "EASY_REORDER",
  "LOW_FRICTION_FOLLOWUP",
  "REVIEW_REQUIRED",
...
export function groupAccelerationByType(
  candidates: QuoteAccelerationCandidate[]
): Record<AccelerationType, QuoteAccelerationCandidate[]> {
  const acc = {
    STALE_QUOTE_REFRESH: [] as QuoteAccelerationCandidate[],
    EASY_REORDER: [] as QuoteAccelerationCandidate[],
    LOW_FRICTION_FOLLOWUP: [] as QuoteAccelerationCandidate[],
    REVIEW_REQUIRED: [] as QuoteAccelerationCandidate[],
  };
