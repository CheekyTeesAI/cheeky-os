...
function appendFromMessages(messages) {
  ...
  for (let index = 0; index < messages.length; index++) {
    ...
    if (!m || !m.to) continue;
    if (isDuplicate(q.items, m.customerId, m.subject, m.text)) continue;
    n += 1;
...
function getItems(statusFilter) {
  initIfMissing();
  const q = readQueue();
  if (!statusFilter) return [...q.items];
...
async function sendQueueSubset(itemsToSend, opts) {
  ...
  const eligibleHotCount =
    typeof opts.eligibleHotCount === "number"
  ...
  if (dryRun) {
    if (moneyMode) {
      console.log(
        `MONEY MODE (DRY RUN): would send ${itemsToSend.length} HOT leads`
      );
    }
    return {
      wouldSend,
...
function hasPendingFollowupForSource(sourceId) {
  ...
  return q.items.some(
    (x) =>
      x.status === "pending_followup" && x.sourceMessageId === sourceId
  );
}
