...
import {
  ...
  type SafetyAction,
} from "../services/safetyGuard.service";
...
function logDecision(label: string, order: Order, action: SafetyAction): void {
  const d = evaluateActionSafety(order, action);
  console.log(
    `[${label}] ${action} -> allowed=${d.allowed}${d.reason ? ` | ${d.reason}` : ""}`
  );
}
...
function main(): void {
  ...
  try {
    assertActionAllowed(deposited, "ORDER_GARMENTS");
  ...
  try {
    assertActionAllowed(jobDone, "CREATE_JOB");
    console.log("assert CREATE_JOB should not run — unexpected OK");
  } catch (e) {
    console.log("assert CREATE_JOB (expected fail):", (e as Error).message);
  }
}
