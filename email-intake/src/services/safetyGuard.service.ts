...
/** Lightweight intent for operator/routes before expensive or risky work. */
export type OperationSafetyIntent = {
  operation?: string;
  ...
  retryCount?: number;
  depth?: number;
  /** When true, `requestedLimit` must be a finite number (still capped at 1000). */
  requireExplicitLimit?: boolean;
};
...
function normalizeOperationKey(op: string | undefined): string {
  return String(op ?? "")
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]/g, "");
...
export function evaluateOperationSafety(
  ...
  if (intent.iterationCount !== undefined) {
    if (
      !Number.isFinite(intent.iterationCount) ||
  ...
  if (intent.retryCount !== undefined) {
    if (
      !Number.isFinite(intent.retryCount) ||
      intent.retryCount < 0 ||
  ...
  if (intent.pageSize !== undefined) {
    if (
      !Number.isFinite(intent.pageSize) ||
      intent.pageSize < 0 ||
  ...
  if (intent.requireExplicitLimit === true) {
    if (
      intent.requestedLimit === undefined ||
      !Number.isFinite(intent.requestedLimit)
    ) {
      return {
        allowed: false,
...
function statusUpper(order: Order): string {
  return String(order.status ?? "").trim().toUpperCase();
}

function isBlocked(order: Order): boolean {
...
function hasDigitizingSignal(order: Order): boolean {
...
function evaluatePublishInvoice(order: Order): SafetyDecision {
  if (!order.isApproved) {
...
function evaluateOrderGarments(order: Order): SafetyDecision {
...
function evaluateRequestDigitizing(order: Order): SafetyDecision {
  if (!order.depositReceived) {
    return { allowed: false, reason: "Deposit has not been received" };
  }
  if (!DEPOSIT_STATUSES_ORDER.has(statusUpper(order))) {
  ...
  if (!hasDigitizingSignal(order)) {
    return {
      allowed: false,
      reason:
        "Digitizing is only allowed when print method is embroidery (EMB) or notes mention embroidery / digitizing / left chest logo",
    };
  }
  return { allowed: true };
}

function evaluateRouteProduction(order: Order): SafetyDecision {
  if (!order.depositReceived) {
    return { allowed: false, reason: "Deposit has not been received" };
  }
  if (!DEPOSIT_STATUSES_ORDER.has(statusUpper(order))) {
...
function evaluateMarkQcReady(order: Order): SafetyDecision {
...
export function assertActionAllowed(order: Order, action: SafetyAction): void {
