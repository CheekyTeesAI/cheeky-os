...
import { calculateOrderPriority } from "./priorityService";

const BUCKET_LIMIT = 25;
const URGENT_CANDIDATE_LIMIT = 50;

export type TodayPrintQueueItem = {
  jobId: string;
  orderId: string;
  customerName: string;
  orderStatus: string;
...
export type TodayNeedsReviewItem =
  | {
      kind: "exception";
      id: string;
      orderId: string | null;
...
export type TodayUrgentOrderItem = {
  orderId: string;
  customerName: string;
  status: string;
...
export type TodayBlockedOrderItem = {
  orderId: string;
  customerName: string;
  status: string;
...
export type TodayActionsResult = {
  printQueue: TodayPrintQueueItem[];
  needsReview: TodayNeedsReviewItem[];
  urgentOrders: TodayUrgentOrderItem[];
  blockedOrders: TodayBlockedOrderItem[];
};

/**
 * Lightweight “today” buckets: capped queries only, no table scans.
 */
export async function getTodayActions(): Promise<TodayActionsResult> {
  try {
  const [
    printRows,
    openExceptions,
  ...
  const printQueue: TodayPrintQueueItem[] = printRows.map((j) => ({
    jobId: j.id,
    orderId: j.orderId,
  ...
  const needsReview: TodayNeedsReviewItem[] = [
  ...
  const urgentOrders: TodayUrgentOrderItem[] = urgentCandidates
    .map((o) => {
  ...
  const blockedOrders: TodayBlockedOrderItem[] = blockedRows
    .filter(
      (o) =>
        o.status === "BLOCKED" ||
  ...
  return {
    ...
    console.error(
      "[todayService] getTodayActions failed; returning empty buckets.",
