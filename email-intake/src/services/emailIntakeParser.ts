...
export interface ParsedEmailIntake {
  ...
  confidence: {
    ...
    garmentType: ConfidenceLevel;
    printMethod: ConfidenceLevel;
  };
...
const QTY_NEAR_GARMENT =
  /\b(\d{1,5})\s*(?:pcs?|pieces?|units?|shirts?|tees?|t-?shirts?|hood(?:ie)?s?|sweatshirts?|garments?|items?)\b/gi;

const QTY_AFTER_PHRASE =
  /\b(?:need|want|order(?:ing)?|looking\s+for|qty\.?|quantity|for)\s*[:\-]?\s*(\d{1,5})\b/gi;

const QTY_FOR_GARMENT = /\bfor\s+(\d{1,5})\s+/gi;

/** Same line / clause: "24 ... tees", "need 12 hoodies" */
const QTY_NEAR_GARMENT_SAME_LINE =
...
function extractPhone(text: string): string | null {
  ...
  if (digits.length === 11 && digits.startsWith("1")) return m[0].trim();
  return m[0].trim();
}

function extractQuantity(
  text: string
): { value: number | null; level: ConfidenceLevel } {
  const hay = text.toLowerCase();
  let m: RegExpExecArray | null;

  const re0 = new RegExp(
    QTY_NEAR_GARMENT_SAME_LINE.source,
    QTY_NEAR_GARMENT_SAME_LINE.flags
  );
  while ((m = re0.exec(hay)) !== null) {
    const n = parseInt(m[1], 10);
    if (n > 0 && n < 100000) return { value: n, level: "HIGH" };
  }
  ...
  while ((m = re2.exec(hay)) !== null) {
    const n = parseInt(m[1], 10);
    if (n > 0 && n < 100000) return { value: n, level: "MEDIUM" };
  }
...
function detectGarment(
  text: string
): { type: string | null; level: ConfidenceLevel } {
  ...
  if (/\bcrew\s*neck\b/.test(t)) return { type: "CREWNECK", level: "HIGH" };
  if (/\bpolos?\b/.test(t)) return { type: "POLO", level: "HIGH" };
  if (/\bt-?shirts?\b|\btees?\b/.test(t)) return { type: "TEE", level: "HIGH" };
  if (/\bshirts?\b/.test(t) && !/sweatshirt|t-?shirt/.test(t))
    return { type: "TEE", level: "HIGH" };
  return { type: null, level: "LOW" };
}

function detectPrintMethod(
  text: string
): { method: string | null; level: ConfidenceLevel } {
  const t = text.toLowerCase();
  if (/\b(?:screen\s*print|screenprinting|silk\s*screen)\b/.test(t))
    return { method: "SCREEN", level: "HIGH" };
  if (/\bdtf\b/.test(t)) return { method: "DTF", level: "HIGH" };
  if (/\bdtg\b/.test(t)) return { method: "DTG", level: "HIGH" };
...
function customerNameFrom(input: EmailIntakeInput): string {
...
export function parseEmailIntake(input: EmailIntakeInput): ParsedEmailIntake {
  ...
  let quantityConf: ConfidenceLevel = qtyRes.level;
  if (quantity === null) {
    assumptions.push("Quantity not clearly stated");
    quantityConf = "LOW";
  }

  let garmentType: string | null;
  let garmentConf: ConfidenceLevel;
  ...
  let printMethod: string | null;
  let printConf: ConfidenceLevel;
  ...
  if (dollars.length === 1) {
    ...
    assumptions.push(
      `Quoted amount taken from explicit price in email: $${quotedAmount}`
    );
  } else if (dollars.length > 1) {
    assumptions.push(
      "Multiple dollar amounts in email; pricing left unset for manual review"
    );
  }
  ...
  if (requiresManualReview) {
    assumptions.push(
      "Manual review recommended (low confidence on one or more key fields)"
    );
  }
