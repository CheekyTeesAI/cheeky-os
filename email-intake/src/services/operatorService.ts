...
export async function getDailyPrintQueue(): Promise<PrintQueueRow[]> {
  try {
...
export type FollowUpPriorityRow = {
  orderId: string;
  status: string;
  customerName: string | null;
  customerEmail: string | null;
  ageHours: number;
  lineItemCount: number;
  recommendedAction: "SEND_ESTIMATE" | "FOLLOW_UP" | "REVIEW_ORDER";
};

function recommendedFollowUp(
  orderId: string,
  status: string,
  lineItemCount: number
): FollowUpPriorityRow["recommendedAction"] {
  const st = status.toUpperCase();
  if (st === "NEEDS_REVIEW") return "REVIEW_ORDER";
  if (st === "QUOTE") {
    if (!hasEstimateBeenDraftedForOrder(orderId) && lineItemCount > 0) {
...
export async function getFollowUpPriority(): Promise<FollowUpPriorityRow[]> {
  try {
...
export async function getHotUnpaidOrders(): Promise<HotUnpaidRow[]> {
  try {
    const rows = await db.order.findMany({
      where: { deletedAt: null },
      include: {
    ...
    const scored = eligible.map((o) => ({
      order: o,
      score: scoreOrderForClosing(o as OrderForClosing),
    }));
...
export async function getOrdersCreatedToday(): Promise<OrderTodayRow[]> {
  try {
...
export async function getNextBestActions(): Promise<NextActionRow[]> {
  try {
    const actions: NextActionRow[] = [];
    ...
    const push = (a: NextActionRow) => {
      if (actions.length >= 10) return;
      if (orderSeen.has(a.orderId)) return;
    ...
    const hotEligible = all
      .filter((o) => isEligibleUnpaidQuote(o as OrderForClosing))
      .map((o) => ({
        order: o,
        score: scoreOrderForClosing(o as OrderForClosing),
      }))
    ...
    for (const { order: ord, score } of hotEligible) {
      ...
      if (orderSeen.has(ord.id)) continue;
      ...
      if (!hasEstimateBeenDraftedForOrder(ord.id) && lines > 0) {
        push({
          type: "SEND_ESTIMATE",
          orderId: ord.id,
          priority: "HIGH",
    ...
    const paidPending = all.filter((o) => {
      if (String(o.status).toUpperCase() !== "PAID") return false;
...
export type OperatorBriefing = {
  printQueue: PrintQueueRow[];
  followUps: FollowUpPriorityRow[];
  hotUnpaid: HotUnpaidRow[];
  ordersToday: OrderTodayRow[];
  nextActions: NextActionRow[];
};

export async function getOperatorBriefing(): Promise<OperatorBriefing> {
  const [printQueue, followUps, hotUnpaid, ordersToday, nextActions] =
    await Promise.all([
      getDailyPrintQueue(),
      getFollowUpPriority(),
      getHotUnpaidOrders(),
