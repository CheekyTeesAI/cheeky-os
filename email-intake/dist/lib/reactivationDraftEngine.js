...
exports.reactivationPriorityGroup = reactivationPriorityGroup;
exports.buildReactivationDraft = buildReactivationDraft;
exports.groupReactivationDraftsByPriority = groupReactivationDraftsByPriority;
exports.buildReactivationDraftsFromCandidates = buildReactivationDraftsFromCandidates;
 ...
 */
function reactivationPriorityGroup(c) {
    if (c.type !== "CUSTOMER_REACTIVATION") {
        return "REVIEW_REQUIRED";
    }
    const { score, band } = (0, followUpScoring_1.scoreFollowUpCandidate)(c);
    if (band === "REVIEW_REQUIRED")
        return "REVIEW_REQUIRED";
    if (band === "HIGH")
...
Close: “I can email a one-pager or quote — what’s the best email to use?”`,
                draftWhy: "High tier: direct but warm reconnection; assumes healthy prior relationship.",
...
“If not now, totally fine — want me to follow up in a few months or leave it?”`,
                draftWhy: "Medium tier: balanced nudge; leaves room for ‘not right now’ without sounding cold.",
...
Keep under ~30 seconds unless they engage.`,
                draftWhy: "Low tier: minimal pressure; good for cooler leads or long quiet periods.",
...
Only call after verifying email/phone in Cheeky OS.`,
                draftWhy: "Score or data is weak — operator must confirm facts, tone, and fit before outreach.",
 ...
 */
function buildReactivationDraft(c) {
    const { score, band } = (0, followUpScoring_1.scoreFollowUpCandidate)(c);
    const pg = reactivationPriorityGroup(c);
    const greet = firstName(c.customerName);
    const { subject, emailBody, callScript, draftWhy, tone } = c.type === "CUSTOMER_REACTIVATION"
...
function groupReactivationDraftsByPriority(drafts) {
    const acc = {
        HIGH_PRIORITY: [],
        MEDIUM_PRIORITY: [],
        LOW_PRIORITY: [],
...
function buildReactivationDraftsFromCandidates(candidates) {
    return candidates
