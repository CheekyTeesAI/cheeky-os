...
exports.suggestIntakeProductionRouting = suggestIntakeProductionRouting;
exports.buildAiIntakeParseResult = buildAiIntakeParseResult;
function asRecord(x) {
...
function extractPrintLocations(text) {
...
function extractDueDateHint(text) {
 ...
 */
function suggestIntakeProductionRouting(input) {
    const t = normLower(input.rawText);
    const qty = input.totalQuantityHint ?? 0;
    const kw = normLower(input.itemTypeKeywords);
    const embRe = /\b(embroid|embroidery|stitch|digitiz|polo|hat|caps?\b|beanie|jacket|left chest logo|patch)\b/;
    if (embRe.test(t) || embRe.test(kw)) {
        return {
            suggestedProductionMethod: "EMBROIDERY",
            routingReason: "Copy mentions embroidery-related garments or embellishment style",
            confidence: "MEDIUM",
        };
    }
    if (/\bscreen\s*print|silkscreen|plastisol\b/.test(t) || /\bscreen\b/.test(kw)) {
        ...
        return {
            ...
            routingReason: "Screen print requested but quantity is below typical screen minimum — operator should confirm or fallback",
            confidence: "LOW",
        };
    }
    ...
    return {
        suggestedProductionMethod: "REVIEW_REQUIRED",
        routingReason: "Insufficient specific method signals — operator should confirm production type",
        confidence: "LOW",
    };
}
/**
 ...
 */
function buildAiIntakeParseResult(input) {
    const rawText = typeof input.rawText === "string" ? input.rawText : String(input.rawText ?? "");
    ...
    const notes = typeof p.notes === "string" ? p.notes : rawText.length ? rawText : null;
    ...
    let reviewRequired = routing.suggestedProductionMethod === "REVIEW_REQUIRED" ||
