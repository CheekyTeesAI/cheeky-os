...
exports.classifyCustomerReply = classifyCustomerReply;
exports.isLikelyCustomerReplyEmail = isLikelyCustomerReplyEmail;
...
function classifyCustomerReply(input) {
...
function isLikelyCustomerReplyEmail(subject, body) {
    const s = String(subject || "").trim();
    const t = String(body || "").trim();
    if (/^re:\s*/i.test(s))
        return true;
    if (/\b(fwd:|fw:)\s*/i.test(s) && /\bproof|mockup|order|deposit\b/i.test(s + t))
        return true;
    const newLead = /\b(quote|estimate|how much|price for|new order|place an order|looking for|need shirts)\b/i.test(t) && t.length > 80;
    if (newLead && !/^re:\s*/i.test(s))
        return false;
