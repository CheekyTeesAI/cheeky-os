...
function firstFailureAlert(alerts) {
    ...
    for (const a of alerts) {
        if (String(a || "").toLowerCase().includes("fail"))
            return String(a);
    }
    return "";
}
    ...
    try {
        ...
        if (text) {
            ...
            if (!intake || intake.success === false || intake.ok === false) {
                return res.status(200).json(fail((intake && intake.error) || "quick-add failed"));
            }
            const parsedCount = Number(intake.data && intake.data.parsedCount) ||
        ...
        if (!revenueCommand || revenueCommand.success === false || revenueCommand.ok === false) {
            return res.status(200).json(fail((revenueCommand && revenueCommand.error) || "revenue command failed"));
        }
        const dashboard = await internalHttpCall("/founder/dashboard", {
            method: "GET"
        });
        if (!dashboard || dashboard.success === false || dashboard.ok === false) {
            return res.status(200).json(fail((dashboard && dashboard.error) || "founder dashboard missing"));
        }
        ...
        const sendPayload = revenueCommand.data &&
            revenueCommand.data.pipeline &&
            revenueCommand.data.pipeline.send;
        const autoSent = sendPayload &&
            sendPayload.data &&
            typeof sendPayload.data.sent === "number"
            ? sendPayload.data.sent
            : 0;
        const runSummary = {
            processed: Number(revenueCommand.data &&
                revenueCommand.data.outreach &&
                revenueCommand.data.outreach.processed) || 0,
            hotLeads: Number(revenueCommand.data &&
                revenueCommand.data.outreach &&
                revenueCommand.data.outreach.hotLeads) || 0,
            messagesGenerated: Number(revenueCommand.data &&
                revenueCommand.data.outreach &&
                revenueCommand.data.outreach.messagesGenerated) || 0,
            readyToSend: Number(revenueCommand.data &&
                revenueCommand.data.queue &&
                revenueCommand.data.queue.approved) || 0,
            autoSent
        };
        const dashboardAlerts = (dashboard.data && Array.isArray(dashboard.data.alerts) && dashboard.data.alerts) ||
            [];
...
=====================`);
        ...
        return res.status(200).json(fail(err instanceof Error ? err.message : String(err)));
    }
});
