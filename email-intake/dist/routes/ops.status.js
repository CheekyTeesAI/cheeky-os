...
router.get("/ops/status", (_req, res) => {
    try {
        ...
        const latestBatchExists = salesOpsOutputs.getLatestOutreachBatchJsonPath() !== null;
        const latestDailySummaryExists = salesOpsOutputs.getLatestDailySummaryJsonPath() !== null;
        const latestRevenueCommandExists = salesOpsOutputs.getLatestRevenueCommandJsonPath() !== null;
        const latestConversionExists = salesOpsOutputs.getConversionsLogPath() &&
        ...
        const pipelineValue = (Number(queue.pending) || 0) * avgOrder +
            (Number(queue.approved) || 0) * avgOrder;
        const nextBestAction = getNextBestCashActions({
            outreach: {
        ...
        if ((Number(queue.failed) || 0) > 0)
            alerts.push("Failed sends need attention");
        if (pendingFollowup > 0)
            alerts.push("Follow-up queue pending");
