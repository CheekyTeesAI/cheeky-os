...
async function seedOrder(squareOrderId, squareInvoiceId) {
    return client_1.db.order.create({
        data: {
...
async function main() {
    ...
    const depData = depositPaidPayload.data;
    ...
    const fullData = fullPaymentPayload.data;
    ...
    console.log(JSON.stringify(await (0, squareWebhookService_1.processSquareWebhook)(p1), null, 2));
    const mid = await client_1.db.order.findUnique({ where: { id: order.id } });
    console.log("Order after deposit:", mid?.status, mid?.amountPaid);
    console.log("--- Full payment event ---");
    console.log(JSON.stringify(await (0, squareWebhookService_1.processSquareWebhook)(p2), null, 2));
    const fin = await client_1.db.order.findUnique({ where: { id: order.id } });
