...
async function main() {
    const order = (await client_1.db.order.findFirst({
        ...
        (await client_1.db.order.findFirst({
    ...
    const fresh = await client_1.db.order.findUniqueOrThrow({ where: { id: order.id } });
    console.log("Using order:", fresh.id, "printMethod:", fresh.printMethod);
    const result = await (0, digitizingService_1.createDigitizingRequestForOrder)(fresh.id);
