...
async function sendApprovalEmail(posts) {
    ...
    if (!to) {
        logger_1.logger.warn("[socialApproval] SOCIAL_APPROVAL_EMAIL missing — skip send");
        return;
    }
    const key = String(process.env.RESEND_API_KEY || "").trim();
    if (!key) {
        logger_1.logger.warn("[socialApproval] RESEND_API_KEY missing — skip send");
        return;
    }
    const from = String(process.env.RESEND_FROM || "").trim() || "Cheeky OS <onboarding@resend.dev>";
    const lines = [];
    ...
    try {
        ...
        if (!res.ok) {
            ...
            logger_1.logger.warn(`[socialApproval] Resend failed: ${res.status} ${t.slice(0, 400)}`);
            return;
        }
        logger_1.logger.info("[socialApproval] approval email sent");
    }
    catch (e) {
        logger_1.logger.warn(`[socialApproval] send error: ${e instanceof Error ? e.message : String(e)}`);
    }
}
