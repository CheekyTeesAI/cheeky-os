...
const warRoom_controller_1 = require("./warRoom.controller");
...
const business_controller_1 = require("../../ops-engine/controllers/business.controller");
...
const SCHEDULE_TEAM = [
    { name: "Patrick", role: "operator" },
    { name: "Employee1", role: "printer" }
];
function invoiceToJobs(invoices) {
    return invoices.map((inv, i) => {
        ...
        const printType = i % 3 === 0 ? "DTG" : i % 3 === 1 ? "DTF" : "Screen Print";
        return {
...
function sortScheduleJobs(a, b) {
    if (a.priority === "rush" && b.priority !== "rush")
        return -1;
    if (b.priority === "rush" && a.priority !== "rush")
        return 1;
...
function blocksNeededForJob(job) {
    const perBlock = job.printType === "DTG" ? 24 : 32;
    return Math.max(1, Math.ceil(job.quantity / perBlock));
}
function taskLabel(printType) {
    if (printType === "DTG")
        return "DTG production run";
    if (printType === "DTF")
        return "Heat press / DTF apply";
...
function buildScheduleFromJobs(jobs) {
    ...
    if (unscheduled.length > 0) {
        bottlenecks.push(`Time risk — ${unscheduled.length} job(s) need more blocks than available (finish lowest priority tomorrow)`);
    }
    return {
        schedule: [
...
async function executeCommand(req, res) {
    ...
    if (!command) {
        return res.status(400).json((0, response_1.errorResponse)("Command not recognized. Try: run-business, run-day, status, followups, close-deals, schedule-day"));
    }
    switch (command) {
        case "run-business":
            return (0, business_controller_1.runBusiness)(req, res);
        case "run-day":
            return (0, operator_controller_1.runDay)(req, res);
        case "status":
            return (0, warRoom_controller_1.getWarRoom)(req, res);
        case "followups":
            return (0, followup_controller_1.autoFollowup)(req, res);
        case "schedule-day": {
            let jobs = [];
            try {
                const invRes = await (0, squareEstimate_service_1.getRecentInvoices)();
            ...
            const priorities = ordered.slice(0, 5).map((j) => ({
                ...
                qty: j.quantity,
                type: j.printType,
                priorityReason: j.priority === "rush"
        ...
        case "generate-revenue": {
            ...
            const outreachTemplate = "Hey, I run Cheeky Tees here in Fountain Inn — we do fast turnaround custom shirts for local crews.\n\n" +
        ...
        case "revive-pipeline": {
            ...
            const reviveTargets = estimatesRes.data
                ...
                const message = urgencyLevel === "WARM"
                ...
                const callScript = `Hey ${r.name}, it’s Patrick from Cheeky Tees.\n\n` +
        ...
        case "close-deals": {
            const topDeals = (0, pipeline_service_1.getNextBestActions)().slice(0, 5);
            const dealsToClose = topDeals.map((deal) => ({
                name: deal.name,
                value: deal.value,
                stage: deal.stage,
                action: "CALL NOW",
                callScript: `Hey ${deal.name}, it’s Patrick from Cheeky Tees.\n\n` +
            ...
            await (0, followup_controller_1.autoFollowup)(req, fakeRes);
            const followupObj = typeof followupPayload === "object" && followupPayload !== null
                ? followupPayload
                : {};
            const followupsTriggered = typeof followupObj.sent === "number" ? followupObj.sent : 0;
            return res.json({
                success: true,
                dealsToClose,
                followupsTriggered
        ...
        default:
            return res.status(400).json((0, response_1.errorResponse)("Invalid command"));
    }
}
